...
- **Killing the worst does.** It kills the render humans liked least on 3 of 5 briefs, where a coin would on 1 in 4, and both misses took the second-worst. What ships wins 53.9%, up 3.9 points, positive on every one of five briefs.
...
Did the kill land on the render humans liked least? Aug 24, five briefs, four renders each.

| Line | Kills on the worst | Hit rate |
|---|---|---|
| Coin | 1 in 4 | 25% |
| Gate | 3 of 5 | 60% |
| Oracle | 5 of 5 | 100% |

A miss is a false positive and a false negative at once, since the wrong render dies and the worst one ships. Both misses, harbor and lantern, killed the second-worst. The truth is pairwise and has one answer here, because on every brief the render with the lowest win rate also lost each of its head-to-heads. Three of five against a coin's one in four is `p = 0.10` on its own, so the test is the table below, which also counts how close a miss came.
