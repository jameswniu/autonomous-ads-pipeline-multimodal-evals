...
|:---|:---|:---|
| Board | Eight mechanical checks, free, before a cent is spent (the product absent before the payoff, the escalation declared, the quirk never spoken by the narration, mouths closed under narration, the centre-crop clause present, every person in a scene written as the story's character, the narration spoken to the viewer, a chain naming the spot's own scenes once each). The four judgment rows it prints are scored when the board is written, before a run starts | The board goes back |
