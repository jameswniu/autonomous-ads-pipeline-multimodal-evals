...
Ten of the thirteen named gating thresholds in [`../probes/`](../probes/) and [`../gates/`](../gates/) are computed from a labelled pass exemplar and a labelled fail exemplar, and [`../evals/derive.py`](../evals/derive.py) holds every one inside the interval its own labels imply.
